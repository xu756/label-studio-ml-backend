...
class YoloModel(LabelStudioMLBase):
    ...
    def setup(self):
        ...
        if not os.path.exists(model_path):
            logger.warning(f"模型文件 '{model_path}' 未找到！请将您训练好的 best.pt 放置在此目录下。")
            self.model = None
        else:
    ...
    def predict(self, tasks: List[Dict], context: Optional[Dict] = None, **kwargs) -> ModelResponse:
        ...
        try:
            from_name, to_name, value = self.label_interface.get_first_tag_occurence('RectangleLabels', 'Image')
        except ValueError:
            logger.error("Label Studio 项目的 Labeling Config 必须包含 <RectangleLabels> 和 <Image> 标签！")
            return ModelResponse(predictions=[])
            ...
            for result in results:
                if result.boxes is not None:
                    for box in result.boxes:
                        ...
                        if not label:
                            logger.warning(f"YOLO 预测类别 '{class_name}' 不在 Label Studio 的标签配置中，跳过该框。")
                            continue
